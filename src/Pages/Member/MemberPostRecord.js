import './Member.css';
import ArticlePostCards from './components/ArticlePostCards';
...
function MemberPostRecord() {
  ...
  const getArticle = async () => {
    try {
      const res = await axios.get(`http://localhost:6001/member/postrecord/${m_sid}`);
  ...
  const delData = async () => {
    const res = await axios.post(
      `http://localhost:6001/member/deletearticlepost`,
      deleteList
    );
    setDeleteList([]);
    console.log(res);
...
export default MemberPostRecord;
